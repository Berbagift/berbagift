...
impl BundleBatchTransferContract {
    ...
    pub fn initialize(env: Env, registry_contract: Address, admin_address: Address) {
        ...
        env.storage()
            .instance()
            .set(&DataKey::CurrentTokenId, &0u32);
    }
    ...
    pub fn set_marketplace_address(env: Env, admin: Address, marketplace: Address) {
        ...
        env.storage()
            .instance()
            .set(&DataKey::MarketplaceAddress, &marketplace);
    }
    ...
    pub fn send_batch_gift(
        ...
        let mut current_token_id: u32 = env
            .storage()
            .instance()
            .get(&DataKey::CurrentTokenId)
            .unwrap();
        ...
        for i in 0..total_recipients {
            ...
            env.storage()
                .persistent()
                .set(&DataKey::NftData(new_item_id), &nft_data);
            env.storage()
                .persistent()
                .set(&DataKey::Owner(new_item_id), &recipient);
            env.storage()
                .persistent()
                .set(&DataKey::TokenUri(new_item_id), &custom_token_uri);

            env.events().publish(
                (
                    symbol_short!("BndlSent"),
                    sender.clone(),
                    recipient.clone(),
                    new_item_id,
                ),
        ...
        env.storage()
            .instance()
            .set(&DataKey::CurrentTokenId, &current_token_id);
    }
    ...
    pub fn send_batch_existing_gift(
        ...
        if total_recipients != token_ids.len()
            || total_recipients != token_amounts.len()
            || total_recipients != messages.len()
        {
        ...
        // Cegah penggunaan NFT yang sedang dijual di marketplace
        let maybe_marketplace: Option<Address> =
            env.storage().instance().get(&DataKey::MarketplaceAddress);
        ...
        for i in 0..total_recipients {
            ...
            if token_amount <= 0 {
                panic!("Jumlah token harus > 0");
            }

            // Periksa apakah NFT ini sedang listed di marketplace
            if let Some(ref marketplace) = maybe_marketplace {
                let is_listed: bool = env.invoke_contract(
                    marketplace,
                    &Symbol::new(&env, "is_nft_listed"),
                    soroban_sdk::vec![&env, token_id.into_val(&env)],
                );
                if is_listed {
                    panic!("NFT sedang dijual di marketplace, batalkan listing terlebih dahulu");
                }
            }
            ...
            env.storage()
                .persistent()
                .set(&DataKey::NftData(token_id), &nft_data);
