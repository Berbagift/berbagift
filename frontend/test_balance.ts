...
async function test() {
  ...
  const targetAddress = "GCVDTFSHBMD7KPKAIHAC5KHZPYPCJ3CA63TW5VTKGYVGA3NRPRSBZMJQ";
  
  const tx = new StellarSdk.TransactionBuilder(account, {
    fee: "100",
    networkPassphrase: StellarSdk.Networks.TESTNET,
  })
  .addOperation(contract.call("balance", StellarSdk.nativeToScVal(targetAddress, { type: "address" })))
  .setTimeout(30)
  .build();
  ...
  if (StellarSdk.rpc.Api.isSimulationSuccess(sim)) {
    ...
    console.log("Balance:", raw, typeof raw);
  } else {
