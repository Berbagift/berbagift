...
export default function InboxPage() {
  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-6 md:pb-8">
      ...
      {/* Category Tabs */}
      <div className="flex items-center w-full overflow-x-auto border border-neutral-200 rounded-md bg-white sidebar-scrollbar">
        {TABS.map((tab, idx) => (
            ...
            <div className="flex-1 min-w-[160px]">
              ...
              >
                ...
                <span className={`rounded-full w-6 h-6 flex items-center justify-center text-xs ml-1 font-semibold ${
                  tab.active ? 'bg-emerald-50 text-emerald-500' : 'bg-neutral-100 text-black'
                }`}>
                  {tab.count}
                </span>
              </button>
      ...
      <div className="border border-neutral-200 rounded-md bg-white py-24 min-h-[400px] flex flex-col items-center justify-center text-center">
        <div className="w-16 h-16 rounded-full bg-neutral-50 flex items-center justify-center mb-4">
          <i className="fi fi-rr-envelope-open text-3xl text-neutral-300 mt-[2px]" />
        </div>
        <h3 className="text-lg font-medium text-black mb-1">You're all caught up</h3>
        <p className="text-neutral-500 text-sm">No new notifications right now</p>
      </div>
    </div>
  );
}
